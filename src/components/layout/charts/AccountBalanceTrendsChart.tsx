...
export function AccountBalanceTrendsChart() {
  ...
  if (!isClient) {
    return (
      <ChartWrapper
        ...
        description="Track your account balances over time"
        isLoading={true}
      >
        <div />
      </ChartWrapper>
  ...
  // Show all accounts (active accounts are already filtered in the hook)
  const filteredAccountNames = sortedAccountNames;

  // Calculate total balance from latest data point
  const latestDataPoint = balanceTrends[balanceTrends.length - 1] || {};
  const totalBalance = filteredAccountNames.reduce((sum, accountName) => {
    return sum + Number((latestDataPoint as BalanceTrendsData)[accountName] || 0);
  }, 0);
  ...
  const footer = (
    <div className="flex items-center justify-between text-sm">
      <div className="flex items-center gap-1">
        ...
        <span className="font-medium">{filteredAccountNames.length}</span>
      </div>
      <div className="flex items-center gap-1">
        <span className="text-muted-foreground">Total Balance:</span>
        <span className="font-medium text-green-600">{formatSummaryAmount(totalBalance)}</span>
      </div>
    </div>
  );
