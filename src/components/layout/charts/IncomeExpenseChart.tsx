...
export function IncomeExpenseChart() {
  ...
  if (error) {
    return (
      <ChartWrapper title="Income vs Expenses" description="Monthly income and expense trends">
  ...
  const totalIncome = financeStats?.monthlyStats.reduce((sum, stat) => sum + stat.income, 0) || 0;
  const totalExpenses =
    financeStats?.monthlyStats.reduce((sum, stat) => sum + stat.expenses, 0) || 0;
    ...
    <div className="flex items-center justify-between text-sm">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-1">
          ...
          <span className="font-medium text-green-600">{formatSummaryAmount(totalIncome)}</span>
        </div>
        <div className="flex items-center gap-1">
          <TrendingDown className="h-4 w-4 text-red-600" />
          <span className="text-muted-foreground">Expenses:</span>
          <span className="font-medium text-red-600">{formatSummaryAmount(totalExpenses)}</span>
        </div>
      </div>
      <div className="flex items-center gap-1">
