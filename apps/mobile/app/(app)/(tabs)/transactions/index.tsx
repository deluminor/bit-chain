...
import type { TransactionsPeriodStats } from '@bit-chain/api-contracts';
...
export default function TransactionsScreen() {
  ...
  // Period stats are calculated server-side across the full date range (not per-page).
  // We lock them on page 1 so they never "shift" when more pages load.
  const [periodStats, setPeriodStats] = useState<TransactionsPeriodStats | null>(null);

  // Convert locked stats (UAH from backend) → user's baseCurrency
  const baseCurrency = useCurrencyStore(s => s.baseCurrency);
  const convertedStats = useConvertedStats(periodStats);

  // Track the latest page for which we accepted items, to deduplicate concurrent fetches.
  const acceptedPageRef = useRef(0);

  const { data, isLoading, isRefetching, isFetching, error } = useTransactions({
  ...
  // Accumulate pages locally to enable infinite scroll while still using useQuery.
  useEffect(() => {
    if (!data) return;

    // Lock stats from the first page of every fresh query. Stats are full-period aggregates
    // returned by the backend regardless of pagination, so page 1 = most authoritative copy.
    if (data.page === 1) {
  ...
  // Reset accumulated items and stats when period / filter / search changes.
  useEffect(() => {
    setPage(1);
    setItems([]);
    setPeriodStats(null);
  ...
  // Use locked stats (page-1 snapshot) converted to baseCurrency.
